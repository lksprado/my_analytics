...
- **conformado** — `int_dates` / `dim_datas`, the conformed date dimension shared by all domains. The spine runs **2019-01-01 → 2050-12-31**; it started in 2020, which silently dropped the first four months of Deusa's patrimônio (set–dez/2019) from `marts.carteira` and `marts.patrimonio_deusa`, because every consumer joins into it with an `INNER JOIN` from its own data. Extending it backwards is additive — nothing uses it as an unbounded spine — but a spine that starts after the data discards rows without a word.
...
- Reconciliação de Deusa: `marts.carteira_deusa` somava R$ 835.455 contra R$ 726.736 de `marts.patrimonio_deusa` em 07/2026 — R$ 108 mil, não os R$ 2 de antes. A diferença é da carteira ter passado a itemizar as seeds de investimentos faltantes (R$ 92 mil) e a Avenue, que não tem coluna na planilha dela. O relatório de meio de mês imprime a diferença; ninguém verificou ainda qual dos dois números está certo.
